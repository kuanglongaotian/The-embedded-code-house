...
void Timer1_Init()
{
  TMOD=TMOD&0x0F;//低位保留，高位清零
	TMOD=TMOD|0x10;//0000 0001，只启动定时器0，并设置为16位定时器/计数器模式
	TF1=0;
	TR1=0;
	TH1=0xFC;//(65535-1000*11.0592/12)/256,设置定时器高8位
	TL1=0x66;//(65535-1000*11.0592/12)%256+1,设置定时器低8位，总共定时1ms
	ET1=1;//设置中断
	EA=1;
}
