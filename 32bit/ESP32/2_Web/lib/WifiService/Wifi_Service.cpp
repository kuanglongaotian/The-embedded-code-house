...
const char *ssid_1 = "408";
const char *password_1 = "19991023";
const char *ssidAP = "ESP32AP";
const char *APpassword = "12345678";

IPAddress myIP;
IPAddress local_IP(192,168,1,123); // 手动设置ESP32联网后的IP,也可以不设置由路由器自动分配
...
u8_t Wifi_Connect(const char *ssid, const char *password)
{
  ...
  WiFi.enableSTA(true);
  WiFi.enableAP(false);
  // Configures static IP address，设置wifi静态ip地址
...
void Wifi_Init()
{
  ...
  while (!Isok)
  {
    ...
    {
    case 0x31:
      // u8_t num=WiFi.scanNetworks();//扫描周围wifi设备
      // if(num!=0)
      // {
      //   for(u8_t i;i<num;i++)
      //   {
